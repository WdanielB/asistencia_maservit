...
export default function SchedulesPanel() {
 ...
 setError('');
 try {
 const response = await fetch('/api/v1/admin/horarios');
 ...
 ? `/api/v1/admin/horarios/${editingSchedule.id}` 
 ...
 try {
 const response = await fetch(`/api/v1/admin/horarios/${id}`, {
 method: 'DELETE'
 });
 const result = await response.json();
 if (result.success) {
 ...
 {error && (
 <div className="glass-panel" style={{ padding: '16px', borderColor: 'var(--accent-error)', color: 'var(--accent-error)' }}>
 ...
 <span style={{ fontSize: '1rem', fontWeight: 600, color: 'var(--accent-warning)' }}>{schedule.minutos_tolerancia} min</span>
 </div>
 <div>
 <span style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', display: 'block' }}>Duración Break</span>
 <span style={{ fontSize: '1rem', fontWeight: 600, color: 'var(--accent-success)' }}>{schedule.break_duracion} min</span>
 </div>
 </div>

 <div style={{ display: 'flex', gap: '8px', borderTop: '1px solid rgba(15,23,42,0.06)', paddingTop: '16px', marginTop: 'auto' }}>
 <button className="btn btn-secondary" onClick={() => handleOpenEditModal(schedule)} style={{ flex: 1, padding: '8px', fontSize: '0.8rem' }}>
 Editar
 </button>
 <button className="btn btn-danger" onClick={() => handleDeleteSchedule(schedule.id)} style={{ flex: 1, padding: '8px', fontSize: '0.8rem', background: 'transparent', border: '1px solid var(--accent-error)', color: 'var(--accent-error)' }}>
 Eliminar
 </button>
 </div>
 </div>
