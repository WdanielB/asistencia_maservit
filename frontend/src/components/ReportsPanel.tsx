...
export default function ReportsPanel() {
  ...
  const fetchLogs = async () => {
    ...
    try {
      ...
      const resLogs = await fetch(`http://localhost:5000/api/v1/admin/marcaciones?${params.toString()}`);
  ...
  const fetchWorkers = async () => {
    try {
      const res = await fetch('http://localhost:5000/api/v1/admin/trabajadores');
  ...
  const handleSaveAdjustment = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/v1/admin/marcaciones/manual', {
        method: 'POST',
      ...
      <div className="glass-panel" style={{ padding: '24px', overflowX: 'auto' }}>
        {loading ? (
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
            ...
            <tbody>
              {logs.map((log) => (
                <tr key={log.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)', transition: 'background 0.2s' }} onMouseEnter={(e) => e.currentTarget.style.background = 'rgba(255,255,255,0.02)'} onMouseLeave={(e) => e.currentTarget.style.background = 'transparent'}>
