...
export default function DashboardHome() {
  ...
  useEffect(() => {
    ...
    const eventSource = new EventSource('http://localhost:5000/api/v1/admin/live-feed');
  ...
  return (
    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '30px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }} className="glass-panel" style={{ padding: '8px 16px', borderRadius: '12px' }}>
      ...
      <div style={{
        ...
        <div className="glass-panel" style={{ padding: '24px', position: 'relative', overflow: 'hidden' }}>
          <div style={{ color: '#34d399', fontSize: '0.9rem', fontWeight: 500 }}>Entradas Hoy</div>
          <div style={{ fontSize: '2.5rem', fontWeight: 800, marginTop: '8px', color: '#10b981' }}>{stats.totalEntradasHoy}</div>
          <div style={{ position: 'absolute', right: '-10px', bottom: '-10px', fontSize: '5rem', opacity: 0.03, fontWeight: 900 }}>🚪</div>
        </div>

        <div className="glass-panel" style={{ padding: '24px', position: 'relative', overflow: 'hidden' }}>
          <div style={{ color: '#f87171', fontSize: '0.9rem', fontWeight: 500 }}>Tardanzas Hoy</div>
          <div style={{ fontSize: '2.5rem', fontWeight: 800, marginTop: '8px', color: '#ef4444' }}>{stats.totalTardanzasHoy}</div>
          <div style={{ position: 'absolute', right: '-10px', bottom: '-10px', fontSize: '5rem', opacity: 0.03, fontWeight: 900 }}>⏱️</div>
        </div>

        <div className="glass-panel" style={{ padding: '24px', position: 'relative', overflow: 'hidden' }}>
          <div style={{ color: '#fbbf24', fontSize: '0.9rem', fontWeight: 500 }}>Breaks Activos</div>
          <div style={{ fontSize: '2.5rem', fontWeight: 800, marginTop: '8px', color: '#f59e0b' }}>{stats.totalBreaksActivos}</div>
          <div style={{ position: 'absolute', right: '-10px', bottom: '-10px', fontSize: '5rem', opacity: 0.03, fontWeight: 900 }}>☕</div>
        </div>

        <div className="glass-panel" style={{ padding: '24px', position: 'relative', overflow: 'hidden' }}>
          <div style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', fontWeight: 500 }}>Ausentes Hoy</div>
          <div style={{ fontSize: '2.5rem', fontWeight: 800, marginTop: '8px', color: '#94a3b8' }}>{stats.ausentes}</div>
          <div style={{ position: 'absolute', right: '-10px', bottom: '-10px', fontSize: '5rem', opacity: 0.03, fontWeight: 900 }}>🚫</div>
        </div>
      </div>
              ...
              {logs.map((log) => (
                <div 
                  ...
                  style={{ 
                    ...
                    gap: '20px',
                    borderColor: log.tipo === 'ENTRADA' ? 'rgba(16, 185, 129, 0.15)' : 'rgba(255,255,255,0.06)'
                  }}
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                    ...
                    <div style={{ 
                      ...
                      {log.foto_scan_url && log.foto_scan_url !== 'manual' ? (
                        <img 
                          src={`http://localhost:5000${log.foto_scan_url}`} 
                          alt="Face capture" 
